...
- **المواصفة:** التنسيق - **القيمة:** PNG مضغوط في ZIP

**عملية التوليد:**
```mermaid
flowchart TD
    A[طلب توليد الأكواد] --> B[اختيار فئة الهدايا]
    B --> C[تحديد العدد والنقاط الثابتة]
...
- إدارة فئات الهدايا (`gift`) وتحديد `points_value` لكل فئة
- عند توليد الأكواد تُنسخ القيمة إلى `qr_codes.points_awarded`
...
- **الإعداد:** احتمالية الفوز - **القيمة:** حسب الرتبة (مثال: 20% / 35% / 50%)
...
**الإصدار:** 1.2 — QR بدون تاجر عند التوليد؛ ربط التاجر وقت المسح + قائمة التجار السابقين
